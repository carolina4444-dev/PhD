...
class TSPEnv(gym.Env):
    ...
    def step(self, action):
        if self.visited[action] or (action == 0 and not all(self.visited[1:])):
            ...
            # Normalize the total distance to fit within 0 to 100
            ...
            # Extra penalty for not visiting all cities
            if not all(self.visited):
                reward -= 50  # Apply a significant penalty for incomplete routes

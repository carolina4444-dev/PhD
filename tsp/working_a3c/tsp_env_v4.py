...
class TSPEnv():
    ...
    def step(self, action):
        # Find indices where action + 1 occurs in self.state
        indices = np.where(self.state == action + 1)[0]

        # Handle case where action + 1 is not found in self.state
        if len(indices) == 0:
            current_index = -1
        else:
            current_index = indices[0]

        # Swap cities if current_index is valid
        if current_index != -1 and current_index != 0 and current_index != self.num_cities:
        ...
        # Recalculate total distance and reward
    ...
    def calculate_reward(self, distance):
        if self.max_distance == self.min_distance:
            return 100  # Return maximum reward if min_distance equals max_distance
